---
description: Audit and archive an execution handoff to prevent stale context pollution
argument-hint: "<execution-slug> [reason]"
...
Read `references/task-specs.md` only when the selected execution handoff contains a Task Binding.

Audit the execution slot and workspace, then prepare an archive plan for one full execution handoff. This command is for handoffs that are done, superseded, abandoned, stale, or failed experiments. It must not move, copy, delete, or relocate files until the user explicitly confirms those actions.

This command applies only to full handoffs under `HandoffDocs/handoffs/`. If `$ARGUMENTS` names a light handoff under `HandoffDocs/light/`, stop and leave it unchanged. Explain that deletion needs explicit confirmation and that archive governance requires a user-requested full conversion.
...
2. Resolve `$ARGUMENTS` to an execution slug and archive reason. If missing or ambiguous, ask before moving anything.
...
10. By default, perform the confirmed archive as a move: create `HandoffDocs/archive/YYYY-MM/<execution-slug>.md`, update the index, then remove the active `HandoffDocs/handoffs/<execution-slug>.md` only after the archived file and index update both succeed.
11. If the user explicitly confirms copy-only/no active-file removal, copy the execution file to the archive path, keep the active file indexed, and mark its active row as `archive-candidate` or `copied-to-archive` instead of adding a final Archived row.
...
14. Leave `HandoffDocs/artifacts/<execution-slug>/` in place unless the user explicitly confirmed artifact cleanup. Mark artifacts historical and not default operational context.
15. For a task-bound execution handoff, update only its `Execution Bindings` path/status after a confirmed move. Do not move the task directory, edit formal spec content, or change the task lifecycle status.
